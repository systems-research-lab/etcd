	...
	go func() {
		...
		for {
			...
			case <-r.ticker.C:
				r.tick()
			case rd := <-r.Ready():
				if confMetaData.Term != rd.CurrentConfMetadata.Term ||
					confMetaData.Index != rd.CurrentConfMetadata.Index {
					r.lg.Info("conf metadata change",
						zap.Any("ConfState", rd.CurrentConfMetadata.ConfState),
						zap.Uint64("Term", rd.CurrentConfMetadata.Term),
						zap.Uint64("Index", rd.CurrentConfMetadata.Index))
					confMetaData = rd.CurrentConfMetadata
				}
