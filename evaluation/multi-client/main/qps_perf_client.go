...
func main() {
	...
	if id == -1 || cluster == "" ||
		...
		thread == -1 || repetition == -1 {
		panic("Wrong arguments.")
	}

	start(id, cluster, warmupTime, cooldownTime, measureTime, thread, repetition, version)
}

type qpsClientReport struct {
	Start   int64   `json:"startUnixMicro"`
	Queries []query `json:"queries"`
}

func start(id int64, cluster string, warmupTime, cooldownTime, measureTime, thread, repetition int64, version string) {
	startTime, queries := sendRequests(id, cluster, warmupTime+cooldownTime+measureTime, thread)
	data, err := json.Marshal(qpsClientReport{startTime.UnixMicro(), queries})
	if err != nil {
		panic("Marshal report error: " + err.Error())
	}

	if err = os.WriteFile(fmt.Sprintf("./qps-client-%v-%v-%v-%v.json", id, thread, repetition, version),
		data, 0666); err != nil {
...
func sendRequests(cid int64, cluster string, runTime, thread int64) (time.Time, []query) {
	...
	for i := int64(0); i < thread; i++ {
		go func(tid int64) {
			...
			for rid := 0; int64(time.Since(startTime).Seconds()) < runTime; rid++ {
				...
				if _, err := clients[rid%len(clients)].Do(context.Background(),
					// clientv3.OpPut(fmt.Sprintf("thread-%v-%v-%v", cid, tid, rid), strconv.Itoa(rid))); err != nil {
					clientv3.OpPut(fmt.Sprintf("thread-%v-%v-%v", cid, tid, rand.Int63()), string(mustRandBytes(512)))); err != nil {
					// cancel()
	...
	queries := make([]query, 0)
	for i := int64(0); i < thread; i++ {
		log.Printf("thread %v returned...\n", i)
		queries = append(queries, <-queryCh...)
	}
	log.Printf("Queries finished.\n")
