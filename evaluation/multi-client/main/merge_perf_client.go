...
func main() {
	...
	if id == -1 || cluster == "" ||
		...
		merge == -1 || thread == -1 || repetition == -1 {
		panic("Wrong arguments.")
	}

	start(id, cluster, merge, warmupTime, cooldownTime, measureTime, thread, repetition)
}

type mergeClientReport struct {
	Start   int64   `json:"startUnixMicro"`
	Queries []query `json:"queries"`
}

func start(id int64, cluster string, merge, warmupTime, cooldownTime, measureTime, thread, repetition int64) {
	startTime, queries := sendRequests(id, cluster, warmupTime+cooldownTime+measureTime, thread)
	data, err := json.Marshal(mergeClientReport{startTime.UnixMicro(), queries})
	if err != nil {
		panic("Marshal report error: " + err.Error())
	}

	if err = os.WriteFile(fmt.Sprintf("./merge-client-%v-%v-%v-%v.json", id, merge, thread, repetition),
		data, 0666); err != nil {
		...
		go func(tid int64) {
			...
			for rid := 0; int64(time.Since(startTime).Seconds()) < runTime; rid++ {
				...
				if _, err := clients[rid%len(clients)].Do(context.TODO(),
					// clientv3.OpPut(fmt.Sprintf("thread-%v-%v", tid, rid), strconv.Itoa(rid))); err != nil {
